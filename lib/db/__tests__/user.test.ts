...
describe('lib/db/user', () => {
  ...
  describe('getOrCreateUser', () => {
    it('crée un utilisateur avec email par défaut si non fourni', async () => {
      const fakeUser = { id: 'db_u1', clerkId: 'clerk_abc', email: 'clerk_abc@unknown.local', name: null }
    ...
    it('utilise l\'email fourni si disponible', async () => {
      const fakeUser = { id: 'db_u2', clerkId: 'clerk_xyz', email: 'user@example.com', name: 'Alice' }
      mockUserUpsert.mockResolvedValue(fakeUser)

      const result = await getOrCreateUser('clerk_xyz', 'user@example.com', 'Alice')
    ...
    it('retourne null pour le nom si non fourni', async () => {
      mockUserUpsert.mockResolvedValue({ id: 'db_u3', clerkId: 'clerk_def', email: 'test@test.com', name: null })
  ...
  describe('getUserByClerkId', () => {
    it('retourne l\'utilisateur si trouvé', async () => {
      const fakeUser = { id: 'db_u1', clerkId: 'clerk_abc', email: 'user@test.com', name: 'Bob' }
      mockUserFindUnique.mockResolvedValue(fakeUser)

      const result = await getUserByClerkId('clerk_abc')

      expect(result).toEqual(fakeUser)
      expect(mockUserFindUnique).toHaveBeenCalledWith({ where: { clerkId: 'clerk_abc' } })
    })
