    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {teams.map((team) => (
            <div key={team.id} className="rounded-lg bg-white p-6 shadow" style={{ borderLeft: `4px solid ${team.color}` }}>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-xl font-bold">{team.name}</h3>
                <div className="h-4 w-4 rounded-full" style={{ backgroundColor: team.color }} />
              </div>
              <div className="mb-4">
                <h4 className="mb-2 text-sm font-medium text-gray-500">Membres ({team.members.length})</h4>
                <div className="space-y-1">
                  {team.members.map((m) => (
                    <div key={m.id} className="text-sm">{m.firstName} {m.lastName}</div>
                  ))}
                </div>
              </div>
            </div>
