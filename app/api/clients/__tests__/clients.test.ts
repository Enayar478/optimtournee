...
const makeRequest = (method: string, body?: unknown, url = 'http://localhost/api/clients') =>
  new Request(url, {
    method,
...
describe('/api/clients', () => {
  ...
  describe('GET', () => {
    ...
    it('retourne la liste des clients', async () => {
      const fakeClients = [{ id: 'c1', name: 'Client A' }, { id: 'c2', name: 'Client B' }]
  ...
  describe('POST', () => {
    ...
    it('crée un nouveau client et retourne 201', async () => {
      const newClient = { id: 'c3', name: 'Nouveau Client', address: '123 rue Test', lat: 48.85, lng: 2.35 }
      mockClientCreate.mockResolvedValue(newClient)

      const req = makeRequest('POST', { name: 'Nouveau Client', address: '123 rue Test', lat: 48.85, lng: 2.35 })
      const response = await POST(req)
      const data = await response.json()
      ...
      expect(mockClientCreate).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ userId: DB_USER.id, name: 'Nouveau Client' }) })
      )
    })
  ...
  describe('PUT', () => {
    ...
    it('met à jour un client existant', async () => {
      ...
      const req = makeRequest('PUT', { id: 'c1', name: 'Client Mis à Jour', address: '456 avenue Test', lat: 48.9, lng: 2.4 })
      const response = await PUT(req)
      const data = await response.json()
  ...
  describe('DELETE', () => {
    it('retourne 401 si non authentifié', async () => {
      mockAuth.mockResolvedValue({ userId: null })
      const req = makeRequest('DELETE', undefined, 'http://localhost/api/clients?id=c1')
      const response = await DELETE(req)
      expect(response.status).toBe(401)
    })

    it('retourne 400 si id manquant', async () => {
      const req = makeRequest('DELETE', undefined, 'http://localhost/api/clients')
      const response = await DELETE(req)
      expect(response.status).toBe(400)
    })
    ...
    it('retourne 500 si Prisma lève une exception', async () => {
      ...
      const req = makeRequest('DELETE', undefined, 'http://localhost/api/clients?id=c99')
      const response = await DELETE(req)
      expect(response.status).toBe(500)
    })
  })
