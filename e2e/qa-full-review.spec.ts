...
const ROUTES = {
  ...
  protected: ["/dashboard", "/planning", "/tournees", "/clients", "/teams", "/settings"],
  ...
  for (const route of ROUTES.public) {
    test(`Public page ${route || '/'} renders correctly`, async ({ page }) => {
      ...
      expect(consoleErrors.filter(e => 
        e.includes("500") || e.includes("404") || e.includes("Error")
      )).toHaveLength(0);
    });
  }
...
test.describe("🔗 Link Validation", () => {
  test("No broken internal links on landing page", async ({ page, context }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
    
    const links = await getAllLinks(page);
    const internalLinks = [...new Set(links.filter(href => 
      href.startsWith(BASE_URL) && !href.includes("#")
    ))];
    
    const brokenLinks: string[] = [];
    
    for (const link of internalLinks.slice(0, 20)) { // Test first 20 to avoid timeout
...
test.describe("📝 Form Functionality", () => {
  test("Sign-in form exists and is accessible", async ({ page }) => {
    ...
    const form = page.locator("form, [data-testid='sign-in-form'], .cl-signIn").first();
    ...
    if (hasForm) {
      ...
      expect(hasEmail || hasPassword, "Sign-in form should have email or password field").toBeTruthy();
    }
  });
  ...
  test("ROI Calculator form elements work", async ({ page }) => {
    ...
    if (hasCalculator) {
      ...
      const inputs = page.locator('input[type="number"], input[type="range"]').all();
...
test.describe("📱 Responsive Design", () => {
  for (const [device, viewport] of Object.entries(VIEWPORTS)) {
    test(`Landing page renders correctly on ${device} (${viewport.width}x${viewport.height})`, async ({ page }) => {
      ...
      if (imageCount > 0) {
        for (let i = 0; i < Math.min(imageCount, 5); i++) {
          ...
          if (isVisible) {
            ...
            expect(box?.width || 0, `Image ${i} should have positive width`).toBeGreaterThan(0);
          }
        }
      }
    });
...
test.describe("🔐 Authentication Flows", () => {
  test("Protected routes redirect unauthenticated users", async ({ page }) => {
    for (const route of ROUTES.protected.slice(0, 3)) {
      ...
      expect(redirectedToAuth || isOnDashboard, 
...
test.describe("📊 Dashboard Components (if accessible)", () => {
  ...
  test("API endpoints return valid responses", async ({ request }) => {
    ...
    for (const endpoint of endpoints) {
      ...
      expect(response.status(), `${endpoint} returned ${response.status()}`).not.toBe(500);
    }
  });
});
