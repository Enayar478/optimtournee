...
import { Calculator, TrendingDown, Clock, Fuel, Leaf, Sparkles } from "lucide-react";
  ...
  return (
    <section ref={ref} className="relative py-32 overflow-hidden">
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.div
            className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 mb-6 text-sm font-medium bg-gradient-to-r from-[#FDF2EB] to-[#FDF2EB]/50 text-[#E07B39] border border-[#E07B39]/20"
          ...
          >
            ...
            <span className="bg-gradient-to-r from-[#E07B39] to-[#F5A572] bg-clip-text text-transparent">30 secondes</span>
          </motion.h2>
        ...
        <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-2">
          {/* Sliders Panel */}
          <motion.div 
            className="rounded-3xl bg-white/80 backdrop-blur-xl p-8 shadow-xl border border-white/50"
            initial={{ opacity: 0, x: -50 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <h3 className="text-2xl font-bold mb-8 flex items-center gap-2">
              <span className="w-10 h-10 rounded-xl bg-[#2D5A3D]/10 flex items-center justify-center">
                <Calculator className="w-5 h-5 text-[#2D5A3D]" />
              </span>
              Votre situation
            </h3>
            ...
            <div className="space-y-8">
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <label className="text-muted-foreground font-medium">Nombre de véhicules</label>
                  <motion.span 
                ...
                <input
                  ...
                  className="w-full h-3 bg-gray-200 rounded-full appearance-none cursor-pointer accent-[#2D5A3D]"
                  style={{
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <label className="text-muted-foreground font-medium">Kilomètres par jour</label>
                  <motion.span 
                    className="text-3xl font-bold text-[#2D5A3D]"
                    key={kmPerDay}
                    initial={{ scale: 1.2 }}
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <label className="text-muted-foreground font-medium">Prix du carburant</label>
                  <motion.span 
                    className="text-3xl font-bold text-[#2D5A3D]"
                    key={fuelPrice}
                    initial={{ scale: 1.2 }}
          ...
          <motion.div 
            ...
            style={{ 
              background: "linear-gradient(135deg, #2D5A3D 0%, #1F3D29 50%, #2D5A3D 100%)"
            }}
            initial={{ opacity: 0, x: 50 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.6, delay: 0.3 }}
          >
            ...
            <motion.div
              className="absolute bottom-0 left-0 w-48 h-48 rounded-full bg-[#4A90A4]/20 blur-3xl"
              animate={{
                scale: [1, 1.3, 1],
                x: [0, -20, 0],
                y: [0, 20, 0],
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: Fuel, label: "Carburant", value: moneySaved, suffix: " €", color: "#F5A572" },
                  { icon: TrendingDown, label: "Kilomètres", value: kmSaved, suffix: " km", color: "#6BB3C7" },
                  { icon: Clock, label: "Heures gagnées", value: hoursSaved, suffix: " h", color: "#F5A572" },
                  { icon: Leaf, label: "CO₂ évité", value: co2Saved, suffix: " kg", color: "#6BB3C7" },
                ].map((stat, index) => (
                  <motion.div
                    key={stat.label}
              ...
              >
                ...
                <motion.button
                  className="w-full py-4 rounded-xl bg-white text-[#2D5A3D] font-semibold shadow-lg"
